        ...
        <div className="max-w-5xl mx-auto pb-20 space-y-8">
             ...
             <div className="text-center">
                <div className="inline-block p-4 bg-white rounded-full shadow-wood-bezel border-4 border-[#d4c5a5] mb-4">
                    <span className="material-icons-round text-elf-red text-5xl">bakery_dining</span>
                </div>
                <h2 className="text-4xl font-serif font-bold text-[#amber-100] text-shadow mb-2 text-white">
                    Wichtel-Bäckerei
                </h2>
                <p className="text-amber-200/80 italic font-serif text-lg">Geheime Rezepte aus der Nordpol-Kantine</p>
             </div>
             ...
             <div className="flex justify-center gap-2">
                 <button 
                    ...
                    className={`px-6 py-2 rounded-full font-bold text-sm shadow-md transition-all border-2 ${
                        filter === 'all' 
                 ...
                 <button 
                    ...
                    className={`px-6 py-2 rounded-full font-bold text-sm shadow-md transition-all border-2 flex items-center gap-2 ${
                        filter === 'food' 
                        ? 'bg-elf-gold text-elf-dark border-yellow-600' 
                        : 'bg-[#fcfaf2] text-[#855E42] border-[#e6dac0] hover:bg-white'
                    }`}
...
const RecipeCard: React.FC<{ recipe: Recipe, index: number, onPrint: (r: Recipe) => void }> = ({ recipe, index, onPrint }) => (
    <div className="bg-[#fcfaf2] p-5 rounded-xl border-2 border-[#e6dac0] shadow-lg relative group hover:border-elf-gold transition-all flex flex-col h-full">
        {/* Number Badge */}
        ...
        <div className="mb-4">
            <h3 className="font-bold text-xl text-elf-dark mb-1 font-serif leading-tight">{recipe.title}</h3>
